...
class APIKeyService:
    ...
    async def revoke_all_user_keys(self, user_id: str) -> int:
        """
        Revoke all API keys for a user (used during subscription downgrade)
        ...
        try:
            ...
            active_keys = self.db.collection(self.api_keys_collection)\
                .where('user_id', '==', user_id)\
                .where('revoked', '==', False)\
                .stream()
            ...
            if count > 0:
                logger.info(f"Revoked {count} API keys for user {user_id} due to subscription downgrade")
            
            return count
            
        except Exception as e:
            logger.error(f"Error revoking all keys for user {user_id}: {e}")
            return 0
