# =============================================================================
# LumTrails Infrastructure - Cloud Run Services (PRODUCTION)
# =============================================================================
# This file defines all 8 Cloud Run services:
...
# 8. Mailing Service (lumtrails-mailing)
#
# PRODUCTION DIFFERENCES:
# - FRONTEND_URL points to your production domain
# - No DEV_WHITELIST_EMAILS (empty = allow all)
# - min_instance_count configurable for warm instances
...
resource "google_cloud_run_v2_service" "main_api" {
  ...
  template {
    containers {
      ...
      # PRODUCTION: Frontend URL is the custom domain
      env {
        name  = "FRONTEND_URL"
        value = local.frontend_url
      }
      # Mailing service URL for authenticated service-to-service calls
      env {
        name  = "MAILING_URL"
        value = "https://lumtrails-mailing-${local.project_number}.${var.region}.run.app"
      }
      # PRODUCTION: No email whitelist - empty string allows all users
      env {
    ...
    # PRODUCTION: Configurable min instances
    scaling {
  ...
  template {
    containers {
      ...
      env {
        ...
        value = "https://lumtrails-external-api-${local.project_number}.${var.region}.run.app"
      }
...
resource "google_cloud_run_v2_service" "pricing_service" {
  ...
  template {
    containers {
      ...
      # Stripe API Key from Secret Manager (PRODUCTION keys)
      env {
        name = "STRIPE_API_KEY"
        value_source {
          secret_key_ref {
      ...
      # Mailing service URL for sending emails
      env {
        name  = "MAILING_URL"
        value = "https://lumtrails-mailing-${local.project_number}.${var.region}.run.app"
      }

      # Stripe Price IDs for PROD
      env {
...
resource "google_cloud_run_v2_service" "mailing_service" {
  ...
  template {
    containers {
      ...
      # Project number for OIDC token verification
      env {
        name  = "PROJECT_NUMBER"
        value = local.project_number
      }
      ...
      env {
        ...
        value = "https://lumtrails-pricing-${local.project_number}.${var.region}.run.app"
      }
